...
class PandocConverter(Converter):
    ...
    def convert(self, article: Article, html: str) -> ConversionResult:
        """Convert article HTML to EPUB using Pandoc.
        ...
        try:
            ...
            if article.author:
                cmd.extend(["--metadata", f"author={article.author}"])

            # Use the work directory for resolving relative image paths
            ...
            logger.info("Pandoc conversion successful: %s", epub_path)
            return ConversionResult(
                epub_path=epub_path,
                title=article.title,
                success=True,
            ...
            error_msg = "Pandoc conversion timed out after 120 seconds"
            ...
            logger.exception("Unexpected error during Pandoc conversion")
            return ConversionResult(
                epub_path=epub_path,
                title=article.title,
                success=False,
